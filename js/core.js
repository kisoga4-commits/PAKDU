...
function startHeartbeatMonitor() {
    setInterval(() => {
        if(db.fraudLogs.length > 0) { triggerSpyBell(db.fraudLogs[0].reason); db.fraudLogs = []; saveToIDB(); }
    }, 5000);
}
...
function openTable(id) { 
    ...
    const u = db.units.find(x => x.id === id); const orderedBox = document.getElementById('ordered-items-bar'); const orderedList = document.getElementById('ordered-items-list'); 
    if(u.orders && u.orders.length > 0) { orderedList.innerHTML = u.orders.map(o => `• ${o.name} <span class="text-[10px] bg-white px-1 rounded">x${o.qty}</span>`).join('<br>'); orderedBox.classList.remove('hidden'); } else { orderedBox.classList.add('hidden'); } 
    switchTab('order'); 
}
...
function handleLockedFeatureClick(isMenuLimit = false) { 
    if(!IS_PRO) { showToast(isMenuLimit ? "⚠️ ฟรีจำกัดการเพิ่มรายการ" : "🔒 เฉพาะ PRO", "error"); openProModal(); return true; } 
    return false; 
}
function handleItemClick(id) { 
    playSound('click'); const u = db.units.find(x => x.id === activeUnitId); let cart = db.carts[activeUnitId]; 
...
function confirmAddonSelection() { 
    let selectedAddons = []; document.querySelectorAll('.addon-checkbox:checked').forEach(cb => { selectedAddons.push({ name: cb.getAttribute('data-name'), price: parseInt(cb.getAttribute('data-price')) }); }); 
    addToCartActual(pendingAddonItem, selectedAddons, currentAddonQty); closeModal('modal-addon-select'); 
}
function addToCartActual(item, addons, qty) { 
    let cart = db.carts[activeUnitId]; let addonText = addons.length > 0 ? ` (${addons.map(a=>a.name).join(', ')})` : ''; let totalAddonPrice = addons.reduce((sum, a) => sum + a.price, 0); let finalPrice = item.price + totalAddonPrice; let finalName = item.name + addonText; 
    let ex = cart.find(c => c.name === finalName && c.price === finalPrice); 
    if(ex) { ex.qty += qty; ex.total += (finalPrice * qty); } else { cart.push({ id: Date.now(), itemId: item.id, name: finalName, price: finalPrice, qty: qty, total: finalPrice * qty, orderBy: 'Master' }); } 
    saveData(); updateCartTotal(); showToast(`เพิ่ม ${finalName} x${qty}`); 
}
function updateCartTotal() { 
    let cart = db.carts[activeUnitId] || []; let t = 0, q = 0; cart.forEach(c => { t += c.total; q += c.qty; }); 
    document.getElementById('cart-total').innerText = t.toLocaleString(); document.getElementById('cart-count').innerText = q; 
}
function reviewCart() { 
...
function editCartItem(idx, d) { 
    ...
    if(cart[idx].qty <= 0) cart.splice(idx, 1); saveData(); 
    if(cart.length === 0) { closeModal('modal-review'); showToast("ตะกร้าว่างเปล่า", "error"); updateCartTotal(); } else { reviewCart(); updateCartTotal(); } 
}
function confirmOrderSend() { 
    const u = db.units.find(x => x.id === activeUnitId); let cart = db.carts[activeUnitId]; 
    if(u.status === 'busy') u.newItemsQty = cart.reduce((s,c)=>s+c.qty, 0); u.status = 'busy'; if(!u.startTime) u.startTime = Date.now(); 
    ...
    db.carts[activeUnitId] = []; saveData(); closeModal('modal-review'); switchTab('customer'); showToast("🚀 ส่งเข้าครัวแล้ว", "success"); 
}

function renderShopQueue() { 
    const box = document.getElementById('shop-queue'); box.innerHTML = ''; const activeUnits = db.units.filter(u => u.orders && u.orders.length > 0).sort((a,b) => a.startTime - b.startTime); 
    ...
    activeUnits.forEach(u => { 
        let t = u.orders.reduce((s,o) => s + o.total, 0); let badge = u.newItemsQty ? `<div class="absolute -top-2 -left-2 bg-red-500 text-white text-[9px] font-black px-2 py-1 rounded-full animate-bounce shadow-md border-2 border-white">สั่งเพิ่ม +${u.newItemsQty}</div>` : ''; 
...
function updateQRDisplay() { 
    ...
    if(navigator.onLine && db.ppay) { offImg.classList.add('hidden'); genArea.innerHTML = ''; new QRCode(genArea, { text: `https://promptpay.io/${db.ppay}/${currentCheckoutTotal}`, width: 150, height: 150 }); txt.innerText = "ดึงข้อมูล Online"; txt.className = "text-[9px] font-bold text-green-500 mt-2"; } 
    else if(db.qrOffline) { genArea.innerHTML = ''; offImg.src = db.qrOffline; offImg.classList.remove('hidden'); txt.innerText = "โหมด Offline (QR ที่ตั้งไว้)"; txt.className = "text-[9px] font-bold text-orange-500 mt-2"; } 
    else { genArea.innerHTML = '<p class="text-[10px] text-gray-400 mt-14">ไม่มีรูป QR</p>'; offImg.classList.add('hidden'); txt.innerText = ""; } 
}
function deleteOrderItem(idx) { 
    const u = db.units.find(x => x.id === activeUnitId); u.orders.splice(idx, 1); 
    if(u.orders.length === 0) { u.status = 'empty'; u.startTime = null; closeModal('modal-checkout'); } saveData(); 
...
function confirmPayment(method) { 
    ...
    if(u.orders.length > 0) { db.sales.push({ id: Date.now(), unitId: u.id, date: getLocalYYYYMMDD(), time: new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' }).split(' ')[1], total: currentCheckoutTotal, method: method, items: JSON.parse(JSON.stringify(u.orders)) }); } 
    u.status = 'empty'; u.orders = []; u.startTime = null; u.newItemsQty = 0; db.carts[activeUnitId] = []; saveData(); closeModal('modal-checkout'); showToast("✅ ชำระเงินเรียบร้อย", "success"); 
...
function calculateCustomSalesRealtime() {
    ...
    let total = 0; db.sales.forEach(sale => { if(sale.date >= document.getElementById('search-start').value && sale.date <= document.getElementById('search-end').value) { total += sale.total; } });
...
function renderAnalytics() {
    ...
    db.sales.forEach(s => { 
        ...
        if(s.date === todayStr) t += s.total; if(IS_PRO && diffDays <= 7) w += s.total; if(IS_PRO && diffDays <= 30) m += s.total; 
        s.items.forEach(i => { let baseName = i.name.split(' (')[0]; itemCounts[baseName] = (itemCounts[baseName] || 0) + i.qty; });
    });
    document.getElementById('stat-today').innerText = t.toLocaleString(); document.getElementById('stat-week').innerText = IS_PRO ? w.toLocaleString() : "🔒"; document.getElementById('stat-month').innerText = IS_PRO ? m.toLocaleString() : "🔒";
...
function saveMenuItem() { 
    const n = document.getElementById('form-menu-name').value.trim(); const p = parseInt(document.getElementById('form-menu-price').value); if(!n || !p) return; 
    const validAddons = tempAddons.filter(a => a.name.trim() !== ''); db.items.push({ id: Date.now(), name: n, price: p, img: tempImg, addons: validAddons }); saveData(); closeModal('modal-menu-form'); showToast("บันทึกเมนูแล้ว", "success"); 
}
function deleteItem(id) { if(confirm("ลบเมนูนี้?")) { db.items = db.items.filter(i => i.id !== id); saveData(); } }

function updateUnits() { 
    const c = parseInt(document.getElementById('config-unit-count').value); const t = document.getElementById('config-unit-type').value; 
    if(!IS_PRO && c > 4) { handleLockedFeatureClick(true); return; } 
    db.unitCount = c; db.unitType = t; forceRebuildUnits(c, t, false); saveData(); showToast("อัปเดตระบบแล้ว", "success"); 
}
function forceRebuildUnits(c, t, bypass = false) { 
    let newUnits = []; 
    for(let i=1; i<=c; i++) { 
        let existing = db.units.find(u => u.id === i); 
        if(!bypass && existing && existing.status !== 'empty') { newUnits.push(existing); } else { newUnits.push({ id: i, status: 'empty', orders: [], startTime: null, newItemsQty: 0 }); } 
    } 
    db.units = newUnits; 
}
...
function applyTheme() {
    ...
    const b = document.getElementById('trial-badge'); const setupRec = document.getElementById('pro-recovery-setup');
    if(IS_PRO) { 
        if(b) b.style.display = 'none'; if(setupRec) setupRec.classList.remove('hidden'); 
        ...
        if(b) b.style.display = 'block'; if(setupRec) setupRec.classList.add('hidden');
...
function handleImage(e, type) { 
    const f = e.target.files[0]; if(!f) return; const r = new FileReader(); r.onload = ev => { 
        if(type === 'logo') { db.logo = ev.target.result; saveSystemSettings(); } else if(type === 'qr') { db.qrOffline = ev.target.result; saveSystemSettings(); } else if(type === 'temp') { tempImg = ev.target.result; const p = document.getElementById('form-menu-preview'); p.src = tempImg; p.classList.remove('hidden'); } 
    }; r.readAsDataURL(f); 
}
function saveSystemSettings() { 
    db.shopName = document.getElementById('sys-shop-name').value; db.theme = document.getElementById('sys-theme').value; db.bgColor = document.getElementById('sys-bg').value; db.bank = document.getElementById('sys-bank').value; db.ppay = document.getElementById('sys-ppay').value; 
    if(document.getElementById('sys-pin').value.trim()) db.adminPin = document.getElementById('sys-pin').value.trim(); saveData(); showToast("บันทึกระบบแล้ว", "success"); 
}
function loadSettingsToForm() { 
    if(!document.getElementById('sys-shop-name')) return;
    document.getElementById('sys-shop-name').value = db.shopName; document.getElementById('sys-theme').value = db.theme; document.getElementById('sys-bg').value = db.bgColor; document.getElementById('sys-bank').value = db.bank; document.getElementById('sys-ppay').value = db.ppay; document.getElementById('sys-pin').value = db.adminPin; 
}
...
document.addEventListener('keydown', e => { 
    if(e.key === 'F12' || (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'J' || e.key === 'C')) || (e.ctrlKey && e.key === 'U')) { e.preventDefault(); showToast("⛔ ไม่อนุญาตให้ใช้เครื่องมือนี้", "error"); triggerSpyBell("DevTools Attempt"); } 
});
